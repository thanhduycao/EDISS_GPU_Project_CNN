    ...
    int padding, int stride) 
{
    ...
    int k = blockIdx.z % KERNEL_COUNT;
    int b = blockIdx.z / KERNEL_COUNT;
    ...
    int padding, int stride) 
{
    ...
    // CORRECT (match the direct kernel):
    int k = blockIdx.z % KERNEL_COUNT;
    int b = blockIdx.z / KERNEL_COUNT;
    ...
    int padding, int stride) 
{
    ...
    int out_x = blockIdx.x * tileSize + tx;
    int out_y = blockIdx.y * tileSize + ty;
    
    int in_x_base = blockIdx.x * tileSize * stride - padding;
    int in_y_base = blockIdx.y * tileSize * stride - padding;
    
    // Load to Shared Memory (Standard Tiling)
    for (int c = 0; c < inputChannels; c++) {
        for (int dy = 0; dy < tileSizeWithPadding; dy += tileSize) {
            for (int dx = 0; dx < tileSizeWithPadding; dx += tileSize) {
                int in_y = in_y_base + ty + dy;
                ...
                if (ty + dy < tileSizeWithPadding && tx + dx < tileSizeWithPadding) {
                    sharedData[c * tileSizeWithPadding * tileSizeWithPadding + (ty + dy) * tileSizeWithPadding + (tx + dx)] = value;
                }
            }
        }
    }
    ...
    if (out_x < outputSize && out_y < outputSize) {
        float sum = 0.0f;
        for (int c = 0; c < inputChannels; c++) {
            // UNROLLING HAPPENS HERE
            // We use #pragma unroll to tell the compiler to expand these loops
            #pragma unroll
            for (int ky = 0; ky < KERNEL_SIZE; ky++) {
                #pragma unroll
                for (int kx = 0; kx < KERNEL_SIZE; kx++) {
                    float in_val = sharedData[c * tileSizeWithPadding * tileSizeWithPadding + (ty * stride + ky) * tileSizeWithPadding + (tx * stride + kx)];
                    float kernel_val = kernels[k * inputChannels * KERNEL_SIZE * KERNEL_SIZE + c * KERNEL_SIZE * KERNEL_SIZE + ky * KERNEL_SIZE + kx];
                    sum += in_val * kernel_val;
                }
            }
        }
        output[b * KERNEL_COUNT * outputSize * outputSize + k * outputSize * outputSize + out_y * outputSize + out_x] = sum;
    }
}
...
void forwardCNN(
    ...
    bool useUnrolled,           // Use new unrolled optimization
    ...
    // In forwardCNN function, replace the convolution launch with:

    dim3 blockDim(8, 8, 1);  // Remove batch from block dimension
    ...
    dim3 gridDim(
        ...
        KERNEL_COUNT * BATCH_SIZE  // Handle batch in grid
    ...
    if (useUnrolled) {
        ...
        convolutionUnrolledKernel<<<gridDim, blockDim, sharedMemSize>>>(
...
int main() {
    ...
    float *h_output_unrolled = (float*)malloc(BATCH_SIZE * KERNEL_COUNT * OUTPUT_SIZE * OUTPUT_SIZE * sizeof(float));
    ...
    float *d_conv_output_unrolled, *d_activation_unrolled, *d_pooling_output_unrolled;
    ...
    CHECK_CUDA_ERROR(cudaMalloc(&d_conv_output_unrolled, BATCH_SIZE * KERNEL_COUNT * OUTPUT_SIZE * OUTPUT_SIZE * sizeof(float)));

    CHECK_CUDA_ERROR(cudaMalloc(&d_activation_unrolled, BATCH_SIZE * KERNEL_COUNT * OUTPUT_SIZE * OUTPUT_SIZE * sizeof(float)));

    CHECK_CUDA_ERROR(cudaMalloc(&d_pooling_output_unrolled, BATCH_SIZE * KERNEL_COUNT * (OUTPUT_SIZE/2) * (OUTPUT_SIZE/2) * sizeof(float)));
    ...
    // Run Unrolled convolution
    
    float unrolled_timing[3] = {0};

    forwardCNN(
        
        d_input, d_kernels,
        
        d_conv_output_unrolled, d_activation_unrolled, d_pooling_output_unrolled,
        
        false, true, // use unrolledred memory
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(h_output_unrolled, d_conv_output_unrolled,
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(h_pooling_output, d_pooling_output_unrolled,
    ...
    // Verify results match between direct and unrolled implementations

    float maxDiffUnrolled = 0.0f;

    for (int i = 0; i < BATCH_SIZE * KERNEL_COUNT * OUTPUT_SIZE * OUTPUT_SIZE; i++) {

        float diff = fabs(h_output_direct[i] - h_output_unrolled[i]);

        maxDiffUnrolled = fmaxf(maxDiffUnrolled, diff);
    ...
    printf("  Unrolled Convolution: %.3f ms\n", unrolled_timing[0]);

    printf("  Speedup: %.2fx\n\n", direct_timing[0] / unrolled_timing[0]);
    ...
    printf("  Max difference between implementations (shared): %e\n", maxDiff);
    printf("  Max difference between implementations (unrolled): %e\n", maxDiffUnrolled);

    if (maxDiff < 1e-5) {

        printf("  [Shared] Results match: YES\n\n");

    } else {

        printf("  [Shared] Results match: NO (possible numerical precision issues)\n\n");

    }

    if (maxDiff < 1e-5) {

        printf("  [Unrolled] Results match: YES\n\n");

    } else {

        printf("  [Unrolled] Results match: NO (possible numerical precision issues)\n\n");

    }

    // Sample output visualization

    printf("[Shared] Sample output feature map (first batch, first channel):\n");
    ...
    printf("[Unrolled] Sample output feature map (first batch, first channel):\n");

    for (int y = 0; y < 5; y++) {

        for (int x = 0; x < 5; x++) {

            printf("%.4f ", h_output_unrolled[y * OUTPUT_SIZE + x]);
    ...
    cudaFree(d_conv_output_unrolled);

    cudaFree(d_activation_unrolled);

    cudaFree(d_pooling_output_unrolled);
